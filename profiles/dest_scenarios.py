...
if __name__ == '__main__':
    ...
    scen = str(sys.argv[-1])
    df_truemodel = pd.read_csv(
        r"./sparseness_data/"
        f"20240418_QD_Exp_Scenario_{scen}.csv", header=0, index_col=0)
    ...
    folder_name = start_date + f"_QD_{scen}_CMAES_Dest"
    os.makedirs(r"./results_2/" + folder_name)
    ...
    with mp.Pool(processes=48) as pool:  # mp.cpu_count() #, maxtasksperchild=1
        for s in seeds:
            np.random.seed(s)
            os.makedirs(fr"./results_2/{folder_name}_{s}/seed_{s}")
            emitters = [
            ...
            # Optimize
            n_iterations = 156  # 125

            prev_archive = None
            itr_improvements = {}

            start_time = time.time()
            print(datetime.now().strftime("%d/%m/%Y %H:%M:%S"), f" Start optimization for Dest {scen} with seed {s}")
            ...
            for itr in range(n_iterations):
                ...
                if itr % 10 == 0:
                    ...
                    file_name = f"./results_2/{folder_name}/seed_{s}" + "/Intermediate_Results_CMAME_Iterations=" + str(
                        itr) + ".pkl"
            ...
            results_opt = dict(archive=archive, emitter=emitters, optimizer=scheduler, convergence=itr_improvements,
                               sim_time=end_time)
            file_name = f"./results_2/{folder_name}/seed_{s}" + "/_Results_CMAME_Iterations=" + str(n_iterations) + ".pkl"
            ...
            plt.savefig(f"./results_2/{folder_name}/seed_{s}/" + str(name) + ".png")
            # plt.show()