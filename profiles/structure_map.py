...
def reformat_solution_for_sim_map(solutions, new_bounds):
    ...
    for i in solutions:
        ...
        for n in new_bounds:
            if isinstance(n, tuple):
                new_value_idx = next(all_index_sol)
                new_value = i[new_value_idx]# Use next() to get the next index
                full_i.append(new_value)
            else:
                full_i.append(n)
...
if __name__ == '__main__':
    ...
    folder_name = start_date + "_QD_0_MAP_Structure"
    ...
    initial_gaussian_center = np.array([np.mean(bounds) for bounds in tuple_bounds])
    initial_gaussian_std = np.array([np.mean(bounds) / 2 for bounds in tuple_bounds])
    ...
    with mp.Pool(processes=1) as pool:  # mp.cpu_count()
        for s in seeds:
            ...
            print(datetime.now().strftime("%d/%m/%Y %H:%M:%S"), f" Start optimization for MAP with seed {s}")

            lock = mp.Manager().Lock()
            for itr in range(n_iterations):
                solutions = scheduler.ask()
                solutions_reformat = reformat_solution_for_sim_map(solutions, new_bounds)
                # print(solutions)
                ...
                if itr % 10 == 0:
                    ...
                    file_name = f"./results/{folder_name}/seed_{s}" + "/Intermediate_Results_MAP_Iterations=" + str(
                        itr) + ".pkl"
