...
if __name__ == '__main__':
    ...
    # # reformat to density -- only once
    # # sort dict on number of nodes
    # sorted_dec_var_db = sorted(dec_var_db.items(), key=lambda item: item[1]["nodes"])
    # dec_var_db = {i: v for i, (k, v) in enumerate(sorted_dec_var_db)}
    #
    # default_input_params = pd.read_excel(r"../complex_stylized_supply_chain_model_generator/input/"
    #                                      r"default_input_params_actors_airsea_qd.xlsx", sheet_name="actors")
    # dec_var = add_input_params_to_db_structures(pd.DataFrame.from_dict(dec_var_db, orient="index"), default_input_params).to_dict(orient="index")

    # # # sort dict on number of density
    # dec_var_db = {k: {**v, "density": nx.density(v["graph"])} for k, v in dec_var_db.items()}
    # sorted_dec_var_db = sorted(dec_var_db.items(), key=lambda item: item[1]["density"])
    # dec_var_db = {i: v for i, (k, v) in enumerate(sorted_dec_var_db)}
    #
    # default_input_params = pd.read_excel(r"../complex_stylized_supply_chain_model_generator/input/"
    #                                      r"default_input_params_actors_airsea_qd.xlsx", sheet_name="actors")
    ...
    # We use reformatting to make the solutions in the range of the bounds
    initial_solution = np.zeros((nr_input_params,))
    initial_bounds = np.array([(-1, 1) for _ in range(nr_input_params)])


    # initial_solution = np.array([np.mean(bounds) for bounds in bounds_parameters])
    ...
    with mp.Pool(processes=1) as pool: #mp.cpu_count()
        for itr in range(n_iterations):
            solutions = scheduler.ask()
            # reformat it for the simulation model -- solutions
            # use same function for processing the results
            solutions_reformat = 0.5 * (solutions + 1) * (bounds_parameters[:, 1] - bounds_parameters[:, 0]) + bounds_parameters[:, 0]
            ...
            if itr % 1 == 0:
                ...
                file_name = "./results/" + folder_name + "/Intermediate_Results_CMAME_Iterations=" + str(
                    itr) + ".pkl"
        ...
        # Save figure
        name = start_date + "_Heatmap_CMAES_Iterations=" + str(n_iterations)  # date.today().strftime("%Y%m%d")
