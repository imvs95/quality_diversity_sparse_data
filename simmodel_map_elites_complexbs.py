...
if __name__ == '__main__':
    ...
    folder_name = start_date + "_QD_0_MAP"
    ...
    initial_gaussian_std = np.array([np.mean(bounds)/2 if (np.mean(bounds) != bounds[0]) and (np.mean(bounds) != bounds[1]) else 0 for bounds in bounds_parameters])
    ...
    with mp.Pool(processes=2) as pool: #mp.cpu_count()
