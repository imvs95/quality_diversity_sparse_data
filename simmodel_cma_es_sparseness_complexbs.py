...
from run_sim_model_obj import run_sim_model
...
if __name__ == '__main__':
    ...
    # this for HPC
    type_sparseness = sys.argv[-2]
    percentage_sparseness = sys.argv[-1]
    ...
    df_true_sparse = add_sparseness(df_in=df_in, type=type_sparseness, percentage=percentage_sparseness,
                                   columns_to_transform=["quantity"], seed=2)
    df_truemodel = aggregate_statistics(df_true_sparse)

    # Import dictionary with decision variables of the graphs
    # with open(r"../complex_stylized_supply_chain_model_generator/data/location_new_graphs_10_betweenness.pkl",
    #           "rb") as file:
    ...
    folder_name = start_date + "_QD_" + type_sparseness + "_" + str(percentage_sparseness)
    ...
    # therefore we choose the mean of the bounds with a sample size of 0.5s
    seeds = range(5)
    emitters = [
    ...
    with mp.Pool(processes=48) as pool:  # mp.cpu_count()
        for itr in range(n_iterations):
            ...
            if itr % 10 == 0:
                ...
                file_name = f"./results/{folder_name}/Intermediate_Results_CMAME_Iterations={str(itr)}" \
                            f"_Sparseness_{type_sparseness}_{percentage_sparseness}.pkl"
        ...
        results_opt = dict(archive=archive, emitter=emitters, optimizer=scheduler, convergence=itr_improvements,
                           sim_time=end_time)
        file_name = f"./results/{folder_name}/Intermediate_Results_CMAME_Iterations={str(n_iterations)}" \
                    f"_Sparseness_{type_sparseness}_{percentage_sparseness}.pkl"
        ...
        # Save figure
        name = f"{start_date}_Heatmap_CMAES_Iterations={str(n_iterations)}" \
                    f"_Sparseness_{type_sparseness}_{percentage_sparseness}"  # date.today().strftime("%Y%m%d")
